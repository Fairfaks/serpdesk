...
// Разбор XML выдачи → { docs: [{url,title}], end: boolean }
...
function parseSerpXml(xmlText) {
  ...
  if (resp.error !== undefined && resp.error !== null) {
    ...
    if (code === 15) return { docs: [], end: true };
  ...
  for (const g of groups) {
    for (const d of g.doc || []) {
      ...
      // Считаем только органику; колдунщики/OneBox-блоки (video, images, news…) пропускаем.
      if (ct && ct !== 'organic') continue;
      const url = String(d.url || '').trim();
      if (!url) continue;
      docs.push({ url, title: String(d.title || '') });
    }
  }
  return { docs, end: docs.length === 0 };
}
