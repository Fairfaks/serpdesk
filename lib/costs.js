...
function estimateProjectRequests(project, keywordCount, selectedEngines = null) {
  const requested = Array.isArray(selectedEngines) ? new Set(selectedEngines) : null;
  const engines = [];
  if (project.cfg.yandex.enabled && (!requested || requested.has('yandex'))) engines.push('yandex');
  if (project.cfg.google.enabled && (!requested || requested.has('google'))) engines.push('google');
