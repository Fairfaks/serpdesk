...
class CheckRunner {
  ...
  async checkKeyword(engine, phrase) {
    ...
    for (let pi = 0; pi < maxPages; pi++) {
      ...
      const { docs, end } = await this.fetchWithRetry(engine, phrase, page);
      for (const d of docs) {
        organicPos++;
    ...
    const competitors = withComp
      ...
      : null;
    if (mine) return { position: mine.position, url: mine.url, error: null, competitors };
    return { position: 0, url: null, error: null, competitors };
  }
...
module.exports = { CheckRunner, Cancelled, FatalRunError };
