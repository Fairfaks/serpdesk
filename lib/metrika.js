...
function parseReport(report, goalsAvailable = true) {
  ...
  for (const row of Array.isArray(report?.data) ? report.data : []) {
    ...
    byEngine[engine].set(key, {
      ...
      goalReaches: goalsAvailable && metrics[5] != null ? (Number(metrics[5]) || 0) : null,
    });
  }
  return {
    byEngine,
...
function reportUrl(counterId, dateFrom, dateTo, includeGoals) {
  const metrics = [
    'ym:s:visits',
    'ym:s:users',
    'ym:s:bounceRate',
  ...
  if (includeGoals) metrics.push('ym:s:favoriteGoalsReaches');
  ...
  return `${REPORT_API}?${params.toString()}`;
}

function isFavoriteGoalsError(error) {
  const message = String(error?.message || error || '');
  return /favoriteGoalsReaches|favorite[_ ]goals/i.test(message);
}

async function queryStats(token, counterId, dateFrom, dateTo) {
  try {
    const report = await getJson(reportUrl(counterId, dateFrom, dateTo, true), token);
    return parseReport(report, true);
  } catch (error) {
    if (!isFavoriteGoalsError(error)) throw error;
    // На счётчике могут быть не выбраны избранные цели. Остальная статистика
    // доступна, поэтому повторяем отчёт без метрики целей и не выдаём ноль.
    const report = await getJson(reportUrl(counterId, dateFrom, dateTo, false), token);
    return parseReport(report, false);
  }
}
